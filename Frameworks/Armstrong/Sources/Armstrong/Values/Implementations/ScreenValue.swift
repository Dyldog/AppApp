...
public final class ScreenValue: EditableVariableValue, ObservableObject {
    ...
    public func value(with variables: Variables, and scope: Scope) throws -> VariableValue {
        ...
        try variables.set(from: arguments.value(with: variables, and: scope) as DictionaryValue)
        ...
        ExpandableStack(scope: scope, title: "Screen") { [weak self] in
            ...
            if let self {
                VStack {
                    ...
                    ForEach(enumerated: (self.name.screen(from: scope)?.initVariables.elements ?? [:]).map { (key: $0.key, value: $0.value) }) { _, element in
