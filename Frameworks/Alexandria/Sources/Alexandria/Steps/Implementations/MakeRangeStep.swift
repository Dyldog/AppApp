...
public final class MakeRangeStep: ValueStep {
    ...
    public func run(with variables: Variables, and scope: Scope) throws -> VariableValue {
        ...
        while try hasPassed(current: next(), step: step, max: end) == false {
            try values.append(next())
            current = try next()
        }
...
public extension MakeRangeStep {
    ...
    convenience init(from decoder: Decoder) throws {
        ...
        try self.init(
            start: container.decode(CodableVariableValue.self, forKey: .start).as((any NumericValue).self),
            end: container.decode(CodableVariableValue.self, forKey: .end).as((any NumericValue).self),
            step: container.decode(CodableVariableValue.self, forKey: .step).as((any NumericValue).self)
        )
    }
